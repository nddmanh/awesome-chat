        ...
        socket.on("chat-text-emoji", (data) => {
            if (data.groupId) {
                ...
                if (clients[data.groupId]) {
                    emitNotifyToArray(clients, data.groupId, socket, "response-chat-text-emoji", response);
                }
            }
            if (data.contactId) {
                let response = {
                ...
                if (clients[data.contactId]) {
                    emitNotifyToArray(clients, data.contactId, socket, "response-chat-text-emoji", response);
                }
            }
        });
