...
function callCreateGroupChat() {
    $("#btn-create-group-chat").unbind("click").on("click", function() {
        ...
        if (countUsers.length < 2) {
            alertify.notify("Một nhóm chat có tối thiểu 3 thành viên.", "error", 7);
            return false;
        }
        ...
        if (!regexGroupChatName.test(groupChatName) || groupChatName.length < 5 || groupChatName.length > 25) {
            alertify.notify("Tên cuộc trò chuyện giới hạn từ 5 đến 25 ký tự và không chứa các ký tự đặc biệt.", "error", 7);
            return false;
        }
...
$(document).ready(function () {
    ...
    socket.on("response-new-group-created", function(data) {
        ...
        socket.emit("member-received-group-chat", {groupChat: data.groupChat._id});
